...
const StatesCreateView = () => {
    ...
    const fields = [
        { name: 'name_en', label: t("state_name"), type: 'text', required: true },
        { name: 'name_ar', label: t("state_name_ar"), type: 'text', required: true },
    ...
    const handleSubmitCreate = async (data,runBeforePush) => {
        let body = {name_en: data.name_en, name_ar: data.name_ar, is_private: "1", country_id: data.country_id,type: 'state'}
        console.log("to send : ", body)
        const response = await createItemInSettings(body);
        if (response) {
            // runBeforePush()
            router.push(paths.dashboard.settings.states);
        }
    }
