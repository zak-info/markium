...
import { useGetCar, useGetCarCostIput, useGetCarMaintenance } from 'src/api/car';
...
export default function CarCostInputTable({ id }) {
  ...
  const [tableData, setTableData] = useState(cost_input?.filter(item => item.car_id == id));
  useEffect(() => {
    setTableData(cost_input?.filter(item => item.car_id == id))
  }, [cost_input])
