...
export default function DocumentListView() {
    const TABLE_HEAD = [
        { id: 'name', label: t('document'), type: "text", width: 140 },
    ...
    const [tableDate, setTableDate] = useState([{
        id: 1, name: "test",attachable:"driver", date: "2023-10-01", actions:(actionMethode)=> <>
            <MenuItem
                onClick={() => {
                    // popover.onClose();
                    actionMethode()
    ...
    return (
        <>
            <ZaityHeadContainer heading={"documents_settings"}
                action={<Button component={RouterLink} href={paths.dashboard.settings.documentsNew} variant="contained" startIcon={<Iconify icon="mingcute:add-line" />}>{t('add_new_document')}</Button>}
                links={[
                    {
                        name: t('dashboard'),
