...
export default function CountriesListView() {
    ...
    let countries =  data?.countries ?  data?.countries?.map(item => ({
        ...item,
    ...
    const [tableDate, setTableDate] = useState(countries.reverse());
    useEffect(() => {
        let items =  data?.countries ? data?.countries?.map(item => ({
            ...item,
            name: item?.translations[0]?.name,
            // country: data?.countries?.find(i => i.id == item.country_id)?.translations[0]?.name,
            actions: (actionMethode) =>
    ...
    return (
        <>
            <ZaityHeadContainer heading={"countries_settings"}
                action={<Button component={RouterLink} href={paths.dashboard.settings.countriesNew} variant="contained" startIcon={<Iconify icon="mingcute:add-line" />}>{t('add_new_country')}</Button>}
                links={[
                    {
                        name: t('dashboard'),
