...
export default function StateListView() {
    ...
    let states =  data?.states ?  data?.states?.map(item => ({
        ...item,
    ...
    const [tableDate, setTableDate] = useState(states.reverse());
    useEffect(() => {
        let sts =  data?.states ? data?.states?.map(item => ({
            ...item,
            name: item?.translations[0]?.name,
            country: data?.countries?.find(i => i.id == item.country_id)?.translations[0]?.name,
            actions: (actionMethode) =>
        ...
        setTableDate(sts?.reverse())
        console.log(sts);
    }, [data])

    return (
        <>
            <ZaityHeadContainer heading={"state_settings"}
                action={<Button component={RouterLink} href={paths.dashboard.settings.statesNew} variant="contained" startIcon={<Iconify icon="mingcute:add-line" />}>{t('add_new_state')}</Button>}
                links={[
                    {
                        name: t('dashboard'),
