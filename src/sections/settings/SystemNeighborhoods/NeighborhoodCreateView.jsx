...
const NeighborhoodCreateView = () => {
    ...
    const fields = [
        { name: 'name_en', label: t("neighborhood_name"), type: 'text', required: true },
        { name: 'name_ar', label: t("neighborhood_name_ar"), type: 'text', required: true },
    ...
    const handleSubmitCreate = async (data,runBeforePush) => {
        let body = {name_en: data.name_en, name_ar: data.name_ar, is_private: data.is_private, country_id: data.country_id,state_id: data.state_id,type: 'neighborhood'}
        console.log("to send : ", body)
        const response = await createItemInSettings(body);
        if (response) {
            // runBeforePush()
            router.push(paths.dashboard.settings.neighborhoods);
        }
    }
    ...
    return (
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
            <CustomBreadcrumbs
                heading={t('add_new_neighborhood')}
                links={[
                    {
                        name: t('dashboard'),
...
export default NeighborhoodCreateView