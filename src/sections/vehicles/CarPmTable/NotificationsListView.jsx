...
import { useGetCar, useGetCarLogs, useGetCarMaintenance, useGetCarPeriodicMaintenance } from 'src/api/car';
...
export default function CarPmListView({ id }) {
  const { enqueueSnackbar } = useSnackbar();

  const { t } = useTranslate();

  const TABLE_HEAD = [
    { id: 'name', label: t('maintenanceName') },
  ...
  const { maintenance } = useGetCarPeriodicMaintenance(id);
                ...
                {dataFiltered
                  ...
                  .map((row) => (
                    <OrderTableRow
                      ...
                      unit_enum={data?.unit_enum.find(item => item?.key == row?.period_unit)?.translations[0]?.name}
